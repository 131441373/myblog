...
新建ipynb, 左侧三横, file,  mount drive按钮(最右)点一下就好了

## template
### Clone Dataset from drive
```
!pip install rarfile
import tarfile
import zipfile
...
```
drive_location = Path('/content/drive/My Drive/ML/Dataset/ACG/images.zip')
copy_location = Path('/content/images.zip')
unzip_location = Path('/content/Dataset')
```

```
# shutil.copyfile(drive_location, copy_location) # copy tree if needed
```
...
class Classifier(PL.LightningModule):
    ...
    def __init__(self, num_classes=10):
        ...
        self.layers = nn.Sequential(
            ...
            nn.LeakyReLU(0.1),
            nn.Linear(200, self.hparams.num_classes),
        )
